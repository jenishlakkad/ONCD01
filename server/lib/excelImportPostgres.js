// PostgreSQL counterpart to server/lib/excelImport.js. NOT wired into any
// real route yet — used only by adminProductsPostgres.js. Only the 3
// DB-touching functions (typeCategories, allSubcategories, and the
// existingSkus lookup inside importWorkbook) actually change — every other
// function here is pure logic, copied verbatim.
...
// PostgreSQL note: `enabled` is a real BOOLEAN column here (confirmed via a
// read-only schema check) — `= TRUE` is used explicitly rather than `= 1`,
// which is SQLite integer-affinity syntax and not something to rely on
// implicitly casting in Postgres.
async function typeCategories(type) {
  return db.all(
    `SELECT c.id, c.name FROM categories c JOIN product_types pt ON pt.id = c.product_type_id
     WHERE pt.key = $1 AND c.enabled = TRUE ORDER BY c.sort_order, c.name`,
    [type]
  );
}

async function allSubcategories() {
  return db.all('SELECT id, name FROM subcategories WHERE enabled = TRUE ORDER BY sort_order, name');
}
...
async function buildTemplate(type) {
  ...
  const cats = await typeCategories(type);
  ...
  if (type === 'jewelry') {
    ...
    info.push(['Subcategory', 'Optional. One of: ' + subs.map((s) => s.name).join(', ')]);
  }
  if (type === 'gemstone') {
...
async function buildStockExport(type, rows) {
  const cols = COLUMN_DEFS[type];
  const headers = cols.map((c) => c.header);

  const [cats, subs] = await Promise.all([typeCategories(type), allSubcategories()]);
  const categoriesById = new Map(cats.map((c) => [c.id, c.name]));
  const subcategoriesById = new Map(subs.map((s) => [s.id, s.name]));
...
async function importWorkbook(type, buffer) {
  ...
  const [categories, subcategories, existingSkuRows] = await Promise.all([
    typeCategories(type),
    allSubcategories(),
    db.all('SELECT sku FROM products'),
  ]);
  const ctx = {
    categories,
    subcategories,
    existingSkus: new Set(existingSkuRows.map((r) => r.sku)),
    seenSkus: new Set(),
  };
